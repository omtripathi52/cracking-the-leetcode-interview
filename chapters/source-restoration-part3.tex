...
extending a sign bit. Because the width is fixed, stopping when \(n\) becomes
zero would lose the positions of the remaining leading zeros.}
...
\(O(n\log n)\); counting takes \(O(n)\) time and constant lowercase-alphabet
space. Use a hash map for a larger or Unicode alphabet.}
...
Each node is visited once: \(O(n)\) time and \(O(h)\) recursion space. An
iterative version can use a queue; one-node and skewed trees are useful tests.}
