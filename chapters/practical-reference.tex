...
All 75 complete solutions were independently syntax-checked under C++11 and
C++17. Each listing was compiled in isolation with representative
judge-provided tree, list, and graph types, so it could not rely on a header or
declaration from another chapter. This verifies completeness and language
compatibility; it is not a substitute for a full automated behavioral test
suite.
\end{conceptbox}
...
\begin{lstlisting}[style=compactcode,caption={Typical linked-list and binary-tree definitions}]
using namespace std;
...
\begin{lstlisting}[style=compactcode,caption={Typical graph-node definition}]
#include <utility>
#include <vector>
...
\begin{revisionbox}{Practice prompts: foundations}
...
\begin{revisionbox}{Practice prompts: advanced patterns}
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash\bfseries}p{2.45cm}
  >{\raggedright\arraybackslash}X
  >{\raggedright\arraybackslash}X@{}}
...
\subsection{A four-pass practice loop}

Use the prompts above as small extensions rather than as new problems to
memorize.

\begin{enumerate}[itemsep=0.3em]
  \item Draw one example and name the state that changes.
  \item Modify the existing solution without looking at its final code.
  \item Test the smallest input, a typical input, and one adversarial case.
  \item Explain the invariant and complexity aloud in under two minutes.
\end{enumerate}

\begin{conceptbox}{When to revisit a lesson}
Return to the original lesson when you can reproduce its code but cannot
explain why an update is safe. The goal of revision is not faster recall of
syntax; it is faster recognition of the state and invariant.
\end{conceptbox}