...
\newcommand{\bookrepository}{https://github.com/omtripathi52/cracking-the-leetcode-interview}
\newcommand{\authoremail}{omt4464@gmail.com}
\newcommand{\authorxhandle}{@omtripathi52}
\newcommand{\authorxurl}{https://x.com/omtripathi52}
  ...
  \ifstrequal{#1}{C++ Implementation}{}{%
    \ifstrequal{#1}{Complete Implementation}{}{%
      \ifstrequal{#1}{Complete Data Structure}{}{%
        ...
        \ifstrequal{#1}{Detailed Visual Dry Run}{\Needspace{20\baselineskip}}{}%
...
\lstdefinestyle{longcode}{
    basicstyle=\ttfamily\fontsize{8.6}{9.6}\selectfont,
    numbers=left,
...
\textcolor{teal}{\textbf{Problem}} &
\textcolor{teal}{\textbf{Difficulty}}\\[-1mm]
\textbf{LC #1} & \textbf{#2}\\[2mm]
...
\phantomsection
{\plexsans\small\bfseries\color{teal}\MakeUppercase{\currentpattern}%
 \ifnum\value{lesson}<7\hfill\textcolor{orange}{GUIDED DEEP DIVE}\fi\par}
\vspace{0.3em}
...
% Two deliberate learning phases make the change in lesson depth visible.
% Each phase receives a quiet divider page; individual lessons still begin on
% their own pages through \problemstart.
\newcommand{\learningphase}[4]{%
  \clearpage
  \phantomsection
  \addtocontents{toc}{\protect\Needspace{5\baselineskip}}%
  \addcontentsline{toc}{part}{#1}%
  \markboth{#1}{#1}%
  \thispagestyle{empty}%
  \vspace*{0.16\textheight}%
  {\plexsans\small\bfseries\color{teal}\MakeUppercase{#2}\par}%
  \vspace{0.65em}%
  {\plexsans\fontsize{31}{37}\selectfont\bfseries\color{navy}#1\par}%
  \vspace{0.55em}%
  {\color{orange}\rule{3.0cm}{2pt}\par}%
  \vspace{1.8em}%
  \begin{tcolorbox}[
    enhanced,colback=softblue,colframe=navy,boxrule=0.8pt,arc=2mm,
    left=5mm,right=5mm,top=4mm,bottom=4mm]
  {\large #3}
  \end{tcolorbox}
  \vfill
  {\plexsans\small\bfseries\color{teal}#4\par}%
...
\vspace{1.1cm}
{\large\bfseries Om Tripathi}\par
\smallskip
{\small\href{mailto:\authoremail}{\authoremail}\quad
\textcolor{teal}{\textbullet}\quad
\href{\authorxurl}{\authorxhandle}}
\end{center}
...
Except where otherwise noted, the prose, diagrams, and book design are
licensed under the Creative Commons Attribution 4.0 International license
(CC BY 4.0). The C++ listings and supporting build source are available under
the MIT License. Attribution details and complete license notices are included
with the repository source.

This is an independent educational publication. It is not affiliated with,
sponsored by, or endorsed by LeetCode. LeetCode and associated problem titles
are trademarks of their respective owners. Problem titles and identifiers are
used only to identify the algorithmic exercises discussed.

The explanations, diagrams, and implementations are presented for educational
purposes. The complete listings were independently syntax-checked under C++11
and C++17 with representative judge-provided types. Readers should still test
behavior and edge cases in the environment where they intend to use the code.
...
\begin{conceptbox}{How the pacing changes}
Part I contains six guided lessons. They demonstrate the complete
reasoning process slowly: interpreting the prompt, drawing the state, tracing
updates, writing pseudocode, and explaining the final implementation. Part II
turns the same method into pattern practice: repeated ideas receive less
repetition while new patterns receive the attention.
\end{conceptbox}
...
\learningphase
  {Part I: Guided Lessons}
  {Learn the complete reasoning process}
  {The first six lessons slow down every decision. Read the prompt, identify
   the changing state, trace a small example, justify the update, and only
   then translate the idea into C++.}
  {SEE THE STATE \quad\textcolor{orange}{\textbullet}\quad TRACE THE CHANGE
   \quad\textcolor{orange}{\textbullet}\quad EXPLAIN THE MOVE
   \quad\textcolor{orange}{\textbullet}\quad WRITE THE CODE}
...
\begin{revisionbox}{Rebuild challenge}
Modify the method to return every unique value pair that reaches the target.
Decide what must change when duplicate values appear, then explain whether a
hash set or a sorted two-pointer scan gives the cleaner result.
\end{revisionbox}
...
\begin{enumerate}[itemsep=0.1em,topsep=0.2em]
    \item Sort the array, then fix one element.
    \item Use two pointers to search for the remaining pair.
    \item Move the left pointer for a small sum and the right pointer for a
    large sum.
    \item Record zero-sum triplets and skip duplicates before continuing.
\end{enumerate}
...
\begin{conceptbox}{State kept during the scan}
...
\begin{revisionbox}{Rebuild challenge}
Add \texttt{countMatches(pattern)} so a wildcard query returns the number of
matching words instead of stopping after the first match. Decide how the DFS
must combine successful branches without counting prefixes as complete words.
\end{revisionbox}
...
\begin{revisionbox}{Rebuild challenge}
Add \texttt{countPrefix(prefix)} so the Trie reports how many inserted words
share a prefix. Decide what extra value each node must store and update it
during insertion without changing the \(O(L)\) operation time.
\end{revisionbox}
...
\begin{lstlisting}[style=longcode,caption={Complete C++ solution: Alien Dictionary}]
#include <algorithm>
#include <queue>
#include <string>
#include <unordered_map>
...
Let \(C=\sum_{i=0}^{n-1}|\texttt{words[i]}|\) be the total number of
...
between them. If \(U\) is the number of unique characters and \(E\) is the
number of distinct directed edges, the additional space is
\(\boxed{O(U+E)}\). Since the language uses only lowercase English letters,
\(U\leq26\).
...
\learningphase
  {Part II: Pattern Practice}
  {Recognize, adapt, and rebuild}
  {The remaining lessons keep the same reasoning structure but move faster.
   Focus on the reusable invariant, compare it with patterns you have already
   seen, and practise rebuilding the implementation without looking.}
  {RECOGNIZE THE PATTERN \quad\textcolor{orange}{\textbullet}\quad ADAPT THE STATE
   \quad\textcolor{orange}{\textbullet}\quad TEST THE EDGES
   \quad\textcolor{orange}{\textbullet}\quad RE-CODE FROM MEMORY}
...
Om Tripathi created \textit{\booktitle} to make data structures and algorithms
easier to see, trace, and rebuild. Each lesson connects an interview pattern
to a concrete example, a visible state change, and a complete C++ solution.
\end{conceptbox}

\subsection{Why This Book Exists}

Interview preparation often turns into memorizing isolated answers. This book
takes the opposite approach: identify the small piece of state an algorithm
maintains, watch that state change on a concrete example, and then rebuild the
implementation from the reasoning. The 75 lessons are grouped so that the same
ideas reappear in increasingly demanding settings.

\begin{revisionbox}{The method behind every lesson}
\begin{enumerate}
  \item Restate the problem in plain language and identify the useful pattern.
  \item Draw a small example and track only the state that can affect the next move.
  \item Explain why each update is safe before translating it into C++.
  \item Re-code the solution without looking, then test edge cases and complexity.
\end{enumerate}
\end{revisionbox}

\subsection{Continue the Project}

This is an open-source digital edition designed to improve over time.
Readers can report mistakes, suggest clearer examples, or contribute focused
corrections through the GitHub repository listed below.

\begin{revisionbox}{Contact and project links}
\begin{tabularx}{\linewidth}{@{}>{\bfseries}p{2.4cm}X@{}}
Email & \href{mailto:\authoremail}{\authoremail}\\[1mm]
X & \href{\authorxurl}{\authorxhandle}\\[1mm]
\ifdefempty{\bookrepository}{}{GitHub & \url{\bookrepository}\\[1mm]}
Licensing & CC BY 4.0 for the book; MIT for code and build source
\end{tabularx}
\end{revisionbox}

\begin{revisionbox}{Author's note}
Do not just read the diagrams. Cover the answer, draw the example yourself,
